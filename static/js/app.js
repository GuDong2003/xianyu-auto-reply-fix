...
// 无法读取 version.txt 时使用中性占位，避免版本号再次硬编码在脚本中
const DEFAULT_VERSION = '未知';
...
// 本地版本信息由 static/version.txt 和 static/update_log.txt 动态生成
...
const UPDATE_LOG_CATEGORY_LABELS = {
    '🎉': '新功能',
    '⚡': '优化',
    '🔧': '修复',
    '📚': '文档'
...
/**
 * 清理 update_log.txt 中沿用的 Python 字符串列表格式。
 */
function normalizeUpdateLogLine(rawLine) {
    return rawLine
        .trim()
        .replace(/^'/, '')
        .replace(/',?\s*$/, '')
...
/**
 * 将 update_log.txt 解析为前端版本历史。
 *
 * 支持发布标题中的可选日期：
 * 发布 v2.0.6（2026-08-14）：更新摘要
 *
 * 旧日志中没有“发布 vX”标题的部分无法可靠判断所属版本，遇到此类
 * 新分组时停止解析，避免把多个旧版本错误归入同一个版本。
 */
function parseLocalUpdateLog(content) {
    const releases = [];
    let currentRelease = null;

    for (const rawLine of content.split(/\r?\n/)) {
        const line = normalizeUpdateLogLine(rawLine);
        const numberedMatch = line.match(/^(\D*?)(\d+)\.\s*(.+)$/u);
        if (!numberedMatch) continue;

        const [, iconText = '', numberText, numberedText] = numberedMatch;
        const icon = iconText.trim();
        const itemNumber = Number(numberText);
        const releaseMatch = numberedText.match(
            /^发布\s+(v\d+(?:\.\d+)+)(?:[（(](\d{4}-\d{2}-\d{2})[）)])?[：:]\s*(.*)$/
        );

        let updateText = numberedText;
        if (releaseMatch) {
            currentRelease = {
            ...
            updateText = releaseMatch[3];
        } else if (itemNumber === 1) {
            // 后续旧日志缺少版本标题，无法安全确定版本边界。
            break;
        }

        if (!currentRelease || !updateText) continue;

        const category = UPDATE_LOG_CATEGORY_LABELS[icon];
        currentRelease.updates.push(category ? `【${category}】${updateText}` : updateText);
    }
...
async function loadLocalVersionMetadata() {
    ...
    const [versionResult, updateLogResult] = await Promise.allSettled([
        fetch(`/static/version.txt?t=${cacheBust}`),
        fetch(`/static/update_log.txt?t=${cacheBust}`)
    ]);

    if (versionResult.status === 'fulfilled' && versionResult.value.ok) {
        const versionText = (await versionResult.value.text()).trim();
        LOCAL_VERSION = versionText || DEFAULT_VERSION;
    } else {
        console.warn('无法读取本地版本文件，使用默认版本');
        LOCAL_VERSION = DEFAULT_VERSION;
    }

    LOCAL_VERSION_HISTORY.version = LOCAL_VERSION;

    if (updateLogResult.status === 'fulfilled' && updateLogResult.value.ok) {
        const updateLogText = await updateLogResult.value.text();
        LOCAL_VERSION_HISTORY.versionHistory = parseLocalUpdateLog(updateLogText);
    } else {
        console.warn('无法读取本地更新日志');
...
async function getUpdateInfo() {
    ...
    // 使用动态加载的本地版本历史作为兜底，不写入远程缓存
    return LOCAL_VERSION_HISTORY;
}
...
function showChangelogModal() {
    ...
    const html = changelog.map(release => {
        const changesHtml = release.changes.map(change => {
            ...
            return `
                <div class="d-flex align-items-start mb-2">
                    ...
                    <span>${escapeHtml(change.text)}</span>
                </div>
            `;
        }).join('');
        ...
        return `
            <div class="changelog-version mb-4">
                <div class="d-flex align-items-center mb-2">
                    <span class="badge bg-primary me-2">${escapeHtml(release.version)}</span>
                    ${release.date ? `<small class="text-muted">${escapeHtml(release.date)}</small>` : ''}
                </div>
        ...
        <div class="text-center text-muted py-4">
            <i class="bi bi-journal-x d-block mb-2" style="font-size: 28px;"></i>
            暂无本地更新日志
        </div>
    `;

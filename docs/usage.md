...
消息过滤规则的“消息来源”可以选择 **AI 准备发送的回复**。AI 生成文本后、消息真正发送前，系统会再次使用这些规则检查回复内容：

- 仅启用“通知人工”时，系统执行通知动作，当前 AI 回复仍会发送。
- 启用“跳过自动回复”或“跳过 AI 回复”时，当前 AI 回复不会发送。
- 选择“全部消息”的规则也会匹配 AI 准备发送的回复。

该检查只作用于 AI 回复，不改变关键词回复、默认回复和指定商品回复的优先级。
